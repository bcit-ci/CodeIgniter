...
class CI_DB_pdo_firebird_driver extends CI_DB_pdo_driver
{
	...
	protected function _update($table, $values, $where, $orderby = array(), $limit = FALSE, $like = array())
	{
		...
		return 'UPDATE ' . $table . ' SET ' . implode(', ', $valstr) . $where . (count($orderby) > 0 ? ' ORDER BY ' . implode(', ', $orderby) : '');
	}
	...
	protected function _limit($sql, $limit, $offset)
	{
		...
		{
			$select = 'FIRST ' . (int)$limit . ($offset > 0 ? ' SKIP ' . (int)$offset : '');
		}
		else
		{
			$select = 'ROWS ' . ($offset > 0 ? (int)$offset . ' TO ' . ($limit + $offset) : (int)$limit);
		}
