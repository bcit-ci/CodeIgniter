...
class CI_DB_sqlsrv_forge extends CI_DB_forge
{
	...
	{
		$sql = ($if_not_exists === TRUE) ? "IF NOT EXISTS (SELECT * FROM sysobjects WHERE ID = object_id(N'" . $table . "') AND OBJECTPROPERTY(id, N'IsUserTable') = 1)\n" : '';
		...
		foreach ($fields as $field => $attributes)
		{
			...
			else
			{
				...
				$sql .= (!empty($attributes['NULL']) && $attribues['NULL'] === TRUE) ? ' NULL' : ' NOT NULL';
		...
		if (is_array($keys) && count($keys) > 0)
		{
			foreach ($keys as $key)
			{
				$key = is_array($key) ? $this->db->escape_identifiers($key) : array($this->escape_identifiers($key));
