...
class Table_test extends CI_TestCase
{
	...
	public function test_set_heading()
	{
		...
		$this->assertEquals(array(array('data' => 'name'), array('data' => 'color'), array('data' => 'size')), $this->table->heading);
	}
	...
	public function test_add_row()
	{
		...
		$this->assertEquals(array(array('data' => 'your'), array('data' => 'pony'), array('data' => 'stinks')), $this->table->rows[1]);
	}
	...
	{
		$expected = array(array('data' => 'name'), array('data' => 'color'), array('data' => 'size'));

		$this->assertEquals($expected, $this->table->prep_args(array('name', 'color', 'size')));

		// with cell attributes
		// need to add that new argument row to our expected outcome
		$expected[] = array('data' => 'weight', 'class' => 'awesome');

		$this->assertEquals($expected, $this->table->prep_args(array('name', 'color', 'size', array('data' => 'weight', 'class' => 'awesome'))));
	}

	public function test_default_template_keys()
	{
		$keys = array('table_open', 'thead_open', 'thead_close', 'heading_row_start', 'heading_row_end', 'heading_cell_start', 'heading_cell_end', 'tbody_open', 'tbody_close', 'row_start', 'row_end', 'cell_start', 'cell_end', 'row_alt_start', 'row_alt_end', 'cell_alt_start', 'cell_alt_end', 'table_close');
	...
	public function test_make_columns()
	{
		...
		$five_values = array('Laura', 'Red', '15', 'Katie', 'Blue');
		...
		$this->assertEquals(array(array('Laura', 'Red', '15'), array('Katie', 'Blue', '&nbsp;')), $this->table->make_columns($five_values, 3));
	}
	...
	public function test_clear()
	{
		...
		$rows = $this->table->make_columns(array('Laura', 'Red', '15', 'Katie', 'Blue'), 3);
	...
	public function test_set_from_array()
	{
		...
		$data = array(array('name', 'color', 'number'), array('Laura', 'Red', '22'), array('Katie', 'Blue'));
		...
		$expected = array(array('data' => 'name'), array('data' => 'color'), array('data' => 'number'));

		$this->assertEquals($expected, $this->table->heading);

		$expected = array(array('data' => 'Katie'), array('data' => 'Blue'),);
	...
	public function test_set_from_object()
	{
		...
		{
			return array(array('name' => 'John Doe', 'email' => 'john@doe.com'), array('name' => 'Foo Bar', 'email' => 'foo@bar.com'),);
		};
		...
		$expected = array(array('data' => 'name'), array('data' => 'email'));

		$this->assertEquals($expected, $this->table->heading);

		$expected = array('name' => array('data' => 'Foo Bar'), 'email' => array('data' => 'foo@bar.com'),);
